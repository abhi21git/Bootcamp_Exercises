...
class NetworkManager {
	...
	func loadEmployees(urlString: String, completion: @escaping (HANDLER)){
        ...
        let sessionTask = session.dataTask(with: request) { (data, response, error) in
            ...
            if error ==  nil {
                ...
                completion(.failure(ServiceError.customError("Please check your internet connection")))
            }
        }
        sessionTask.resume()
    }
	...
	func logIn(urlString: String, parameters: Data, completion: @escaping(HANDLER)) {
		...
		let sessionTask = session.dataTask(with: request) { (data , response , error) in
			...
			if error == nil {
				do {
					...
					completion(.success(model as Any))
				}
				
			}
			else {
				completion(.failure(ServiceError.customError("Could not login.")))
			}
		}
		sessionTask.resume()
	}
    ...
    func profileApi(urlString: String, parameters: [String : String], completion: @escaping(HANDLER)) {
        ...
        let sessionTask = session.dataTask(with: request) { (data, response, error) in
            ...
            if error == nil {
                ...
                completion(.failure(ServiceError.customError("Please check your connection.")))
            }
        }
        sessionTask.resume()
    }
	...
	func googleImageSearch(urlString: String, completion: @escaping(HANDLER)) {
		...
		let sessionTask = session.dataTask(with: request) { (data, response, error) in
			...
			if error ==  nil {
				...
				completion(.failure(ServiceError.customError("Please check your internet connection and try again.")))
			}
		}
		sessionTask.resume()
	}
